...
import { aiChatPlugin } from "@/components/editor/kits/ai-kit"

import { discussionPlugin } from "@/components/editor/kits/discussion-kit"
			...
			if (sample === "comment") {
				...
				for (const block of blocks) {
					for (const chunk of block) {
						...
						controller.enqueue(
							encoder.encode(`data: ${chunk.texts}\n\n`)
						)
					}
				}
...
const createCommentChunks = (editor: PlateEditor) => {
	...
	const blocks =
		...
		(editor.api.isExpanded() || isSelectingSome)
			? selectedBlocks
			: editor.children
